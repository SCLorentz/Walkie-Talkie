...
async fn print_devices(client: &Client) -> Result<(), Box<dyn Error>>
{
	let user_id = client
		.user_id()
		.unwrap();
	...
	let own_device = client
		...
		.get_own_device()
		.await?
		.expect("We should have our own device by now");
...
pub async fn login(proxy: Option<Url>) -> Result<(), Box<dyn Error>>
{
	...
	println!("$ grim -g \"$(slurp)\" - | zbarimg --oneshot -Sbinary PNG:- | base64 -w 0 | wl-copy");
	...
	let data = QrCodeData::from_base64(input);

	let QrCodeModeData::Reciprocate { server_name } = &data.unwrap().mode_data else {
		return Err(
			Box::new(
				"The QR code is invalid, we did not receive a homeserver in the QR code."
					.parse::<u32>()
					.unwrap_err()
			)
		);
	...
	let _login_client = oauth.login_with_qr_code(Some(&registration_data));
	//let mut subscriber = login_client.subscribe_to_progress();

	/*let task = std::thread::spawn(async move || {
	...
	println!("Successfully logged in as {user_id} using the qr code, cross-signing status: {status:?}");
